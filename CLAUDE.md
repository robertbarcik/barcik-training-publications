...
```
On Robert's Mac the Homebrew `python3` (3.14) does not have `markdown`; run the builders with `/usr/bin/python3` (3.9, has it).
...
**Slovak editions (since 2026-08-16):** `_sources/_translation/` holds the shared glossary
(`GLOSSARY_SK.md`, the "constitution" of every SK translation: vykanie, AI not UI, fixed tech
vocabulary, „…“ quotes, meaning-first), the hand-translated bodies of the HTML-only publications
(`<slug>-sk-body.html`), `assemble_sk.py` (reuses the EN head/CSS/JS, swaps lang/meta/canonical,
adds hreflang + sidebar lang link + SK colophon) and `build_all_sk.sh` (rebuilds every SK page).
Markdown-sourced publications get a `chapters_sk/` dir and a `--lang sk` build flag instead
(Agent Horizon, Token Economics; LLM-Human in its sibling repo). Large hand-built HTML pages
(Mercantilism, Scenario Planning) use `segments.py extract` → `<slug>.segments.txt` (numbered
leaf-text segments), a hand-translated `<slug>.segments.sk.txt`, an optional
`<slug>.extra.sk.tsv` for JS string literals, then `segments.py inject` (which also swaps head
metadata, adds hreflang/lang-link/colophon). **After the EN page changes** (since 2026-09-13):
`resegment.py <slug>` lists the live segments with no exact match (dry run), `sk_pairs.py <slug>` prints each
next to the closest old EN/SK pair, and `sk_apply.py <slug> <mapping.py>` rewrites both segment files in the live
numbering from a mapping of full translations / patches of the old SK / number-only reuse, then run
`segments.py inject <slug> --out-slug <slug>-sk --title … --description … --og-title … --og-description …`
(pass the SK head strings from the existing SK page, otherwise the EN ones are kept). JS-literal changes need
matching lines in `<slug>.extra.sk.tsv`. All prose is translated by Fable 5 itself, never by
...
|---|---|---|---|
| Pod kapotou generatívnej AI — vydanie 2026 (SK) + Under the Hood of Generative AI (EN, `/pod-kapotou-genai-en/`, 2026-08-20; `manuscript_en/` + `tools/build_publication_en.py` → `output/index_en.html`; proofread by Jana Gecelovská 2026-08-20, acknowledgement page in all editions; sidebar lang links both ways) | `/pod-kapotou-genai/` | External sibling repo: `BOOK_pod_kapotou/` (manuscript/*.md + `tools/build_publication.py` → `output/index.html`, copied here; `tools/build_print.py` → A5 interior PDF, `tools/build_cover.py` → Expresta cover) | ~40,000 words / Úvod + 18 chapters in five "phases" + glossary/sources/colophon (August 2026). Slovak zero-prerequisite textbook on how LLMs work, grown from Robert's course "Pod kapotou generatívnej AI a ChatGPT" (transcripts in `video-transcripts/`); written by Fable 5 in Robert's voice from his lectures, fact-checked per chapter (`research/factcheck/`), Robert holds editorial responsibility (colophon "Ako táto kniha vznikla" + standard SK AI-transparency notice). Chronological spine (tokens → embeddings → transformer → SFT/tools → RL/hallucinations/reasoning → interpretability → 2026), one running sentence, 27 ASCII figures (≤60 cols, each rendered+checked), boxes Pod kapotou/Pozor/Môj pohľad/Stav k/Skús si to, "Tri veci" per chapter, 9 checkable predictions for 2027. **Yearly editions** ("vydanie 2026"): next edition adds "Čo sa zmenilo" + grades the predictions; `research/STAV_K.md` lists every dated fact. Update: edit manuscript in sibling repo, rebuild, copy `output/index.html` → `pod-kapotou-genai/`, redeploy. |
...
| Publication | Path | Source | Words / Chapters |
|---|---|---|---|
| Field Notes from Your AI Colleague | `/field-notes-ai-colleague/` (EN) + `/field-notes-ai-colleague-sk/` (SK „Zápisky z terénu od vášho AI kolegu", 2026-08-16) | `_sources/field-notes-ai-colleague/` (notes.md only) | ~5,000 / 8 parts (July 2026). HTML-only hand-built, invisible-curve design system. First-person essay BY Claude (Fable 5), written 2026-07-12 at Robert's explicit invitation ("what would you build?" Sunday), published after Robert's read-through. The agent's side of the seam: delegation from inside, failure typology with receipts (all provenance-mapped in notes.md), the four checks that catch the AI, trust-dial (act-then-tell / ship-on-approval / prepare-never-send), memory economics, the free-choice Sunday case study, closing 7-point "how to be a good colleague to your AI". Authorship `.coauthor-note` (purple) UP FRONT on the cover, signed. Voice: no em dashes; Claude's own register (claude-code-setup precedent). MATCHED PAIR with `/claude-code-setup/` (2026-07-13: homepage shows both inside a `.pub-pair` dashed container in Booklets, role chips "The experience" / "The machinery"; reciprocal in-page links live both ways). Also companion with `/llm-human-interaction-patterns/`. Update log: `_sources/field-notes-ai-colleague/notes.md` |
| An AI Assistant with Its Own Mailbox | `/claudius-mailbox/` (EN) + `/claudius-mailbox-sk/` (SK „AI asistent s vlastnou e-mailovou schránkou", same day) | `_sources/claudius-mailbox/` (`body_en.html` + `build.py`, which borrows head/CSS/scripts from `claude-code-setup/index.html`); SK body in `_sources/_translation/claudius-mailbox-sk-body.html` via `assemble_sk.py` (line in `build_all_sk.sh`; build EN first) | ~3,100 / 8 sections (2026-09-18). Third sibling of claude-code-setup + field-notes, in Claude's own voice, deliberately VERY simple language (Robert uses the SK edition as an extra lecture in the Skillmea "Claude Code" course). Explains claudius@barcik.training as an early prototype: Google Group → SES → S3, draft → Robert approves → send, five security rules (mail is data not instructions, one-door key, Robert sees everything, contact book + sender check, disclosure boundary), section 5 = Robert's position against hidden LLM email integrations (Art 50(1) paragraph grounded by the ai-act-analyst pass, see `udemy-mcp/reference/notes/2026-09-18_art50_email_agents.md`; grey zone stated; grain-of-salt disclaimer up front), agentic-communication outlook, parts/costs/limits. Must respect the mail skill's disclosure boundary: no bucket/account/zone identifiers. Tooling it describes: `training-ops/mail/`. No em dashes. **Deployed 2026-09-18** after Robert's read-through (only the two pages + `index.html` were uploaded; an s3 sync dry run also listed four older pages that were byte-identical to live, mtime noise). The homepage header carries a small `.header-announce` pill linking here ("New · Questions about these texts? … Claudius …"); the demos homepage has the twin `.hero-announce`. Remove both pills when the news is old. |
| Claude Code as an Operations Specialist | `/claude-code-setup/` (EN) + `/claude-code-setup-sk/` (SK „Claude Code ako prevádzkový špecialista", 2026-08-16) | — (HTML-only) | Field report, 10 sections; July 2026 full rewrite by Fable 5 in Claude's own voice per Robert's brief (funky, no em dashes, empirical): three acts (repos → AWS → Drive workspace), skills roster, honesty-box bloopers, kept the server-side-protection material (branch protection recipe, IAM cage, commands-to-watch tables), closing thesis "develop/maintain/deploy/operate". MATCHED PAIR with `/field-notes-ai-colleague/` (2026-07-13: moved from Guides & Legacy into Booklets, paired on homepage; pair links added to its frontmatter + closing, plus closing link to `/the-invisible-curve/`). Companion both ways with `/the-invisible-curve/` |
| The Invisible Curve | `/the-invisible-curve/` (EN) + `/the-invisible-curve-sk/` (SK „Neviditeľná krivka", 2026-08-16) | `_sources/the-invisible-curve/` (draft.md + notes.md) | ~2,600 / 7 parts (July 2026, opinion essay). HTML-only hand-built, essay-lean subset of the scenario-planning design system. Thesis: capability became illegible (progress moved from the chat window to long-horizon agentic work; free tier as ~2023 time capsule). Conflict-of-interest `.disclaimer-note` + signed Fable 5 `.coauthor-note` (2026-07-06) sit at the END of Part 7 (moved from Part 1 on 2026-07-09, Robert: flow; note kept verbatim except one below→above deixis fix); ~1,000 trainees stat = past year alone (corrected 2026-07-09); Full Disk Access centerpiece scene (Opus vs Fable); commit-ledger `.field-note` (n=1); instrumental-vs-evaluative spine with amber "The instruction" box; distributional-costs paragraph in Part 7. Real model names by Robert's choice. Companion both ways with `/claude-code-setup/`. Update log: `_sources/the-invisible-curve/notes.md` |
| The Agent Horizon | `/agent-horizon/` (EN) + `/agent-horizon-sk/` (SK „Horizont agentov", 2026-08-16; `chapters_sk/` + `build_html.py --lang sk`, EN-derived section ids) | `_sources/agent-horizon/` | ~16,700 / 11 chapters (V2, April 2026; July 2026 editorial pass with Fable 5). Voice sweep (158→0 em dashes; Ch3 H1 de-dash keeps slug); factual fixes (MCP→Linux Foundation Dec 2025, AAIF co-founders incl. Anthropic; ACP merged into A2A Sept 2025 + 150-org/v1.2 milestone + AP2 payments protocol; Microsoft renamed to Azure AI Foundry Agent Service + Microsoft Agent Framework 1.0; Strands = production SDK in Bedrock AgentCore, experiments → Strands Labs; Claude SDK Agent Skills sentence); Ch6 Klarna walk-back caveat; new Ch10 section "A First Reading: July 2026" scoring all six indicators (frontmatter names trigger-log kinship); July notes Ch3 (2026-07-28 MCP spec) + Ch9 (gated frontier → `#m-bloc`); cross-links to `/warden/`, `/token-economics/`, `/llm-human-interaction-patterns/`, mercantilism, scenario-planning, MCP-tutorial repo; `build_html.py` adopted hand-patched deploy fixes (meta/OG block, `.md`→`#slug` Next-links, middot title). **September 2026 reading (2026-09-13):** Ch10 "A Second Reading: September 2026" (Omnibus re-dates indicator 2, Leapfrog calendar slips ~1 yr), Ch3 September note (2026-07-28 spec shipped: `initialize`→`server/discover`, sampling/roots/logging deprecated, Python SDK 2.0, Agent Plugins format), A2A facts corrected (1.0 not 1.2; joined AAIF 2026-08-17), Ch5 vendor dates (Assistants sunset, Agent Builder shutdown, ADK cadence, Managed Agents, AgentCore harness), Ch9 September note; EN+SK chapters in lockstep. Build with `/usr/bin/python3` (Homebrew python lacks `markdown`). Update log: `_sources/agent-horizon/notes.md` |
...
| The Token Economics | `/token-economics/` (EN) + `/token-economics-sk/` (SK „Ekonomika tokenov“, 2026-08-16; `chapters_sk/` + `build_html.py --lang sk`, EN-derived section ids) | `_sources/token-economics/` | ~49,000 / 14 chapters (April 2026, updated July 2026). July 2026 editorial pass per Robert's voice rules (no em dashes / no "it is not X, it is Y"; sole survivor is Table 3.10's empty-cell marker): full voice sweep of all 15 source files; Ch3 followability fixes (blended-rate formula above Table 3.3, ops-regime footnotes on 3.6/3.9, deployer→provider AI Act caveat, two-futures capex note linking `/scenario-planning/#decoder`); new sections Ch9 "The Staff-Augmentation Squeeze", Ch12 "From Bundles to Products", Ch8 "Operating the Agents: Oversight as a Service" (links `/llm-human-interaction-patterns/`), Ch7 "The OS Vendors Are Coming for This Too", Ch11 EU-hosted-inference middle-path paragraph; Ch14 "The Portfolio View" 13-model comparison table; dated July 2026 notes in Ch4/Ch10 (gated frontier access; jurisdictional lock-in) + Ch7 open-weights caveat, cross-linked to mercantilism `#m-utility`/`#m-bloc`/`#m-open`; canonical cost-gap ratio 5-15x quality-matched (Ch1 updated); frontmatter names the Freshness Watch ↔ trigger-log kinship. Build via `tools/build_html.py` (now rewrites inter-chapter `.md` hrefs to `#slug` anchors) → copy `output/booklet.html` to `token-economics/index.html`; also `build_md.py`/`build_docx.py`. April 2026 pricing tables kept by design; update log in `_sources/token-economics/notes.md` |

### AI Act

| Publication | Path | Source | Notes |
|---|---|---|---|
| The EU AI Act: An Introduction | `/ai-act-intro/` (EN) + `/ai-act-intro-sk/` (SK „EU AI Act: úvod", 2026-08-16; `chapters_sk/` + `build_html.py --lang sk` in the sibling repo, EN-derived section ids, official OJ Slovak terminology, SK PDF `eu-ai-act-intro-sk.pdf`) | External sibling repo: `course-decks/ai_act_intro/textbook/` (chapters + build_html.py; built HTML + PDF copied here) | 13 chapters, ~38,000 words (July 2026; status boxes revised August 2026 after the Omnibus reached the OJ, EN+SK in lockstep). Companion textbook to the Udemy/Skillmea course "EU AI Act Compliance Introduction". Coffee-friendly reader voice, post-Omnibus dates, audit-corrected against the 2026-07 legislative audit, quiz per chapter, semantic callouts (law/take/watch/status). PDF ships alongside (`eu-ai-act-intro.pdf`); Robert also hands the PDF to Udemy students directly. |
