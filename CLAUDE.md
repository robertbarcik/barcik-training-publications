...
| The Mercantilism of Generative AI | `/mercantilism-of-genai/` | `_sources/mercantilism-of-genai/` (notes.md only) | ~9,600 words / 14 sections (June 2026); HTML-only hand-built, cloned from scenario-planning design system; 6 static CSS/SVG diagrams (incl. "two futures" armed↔targeted spectrum + three-tier shield); new `.disclaimer-note` (red), `.bet`, `.coauthor-note`, `.field-note` (teal author field-report card) components; per-industry impact subsection + first-person author field report, dated falsifiable bets + signed AI co-author's note + trigger-log time capsule. July 2026 log update: gated return recorded (Mythos vetted-companies-only primarily US; Fable expensive API + moderation layer), Bet 1 scored won early (green "Scored" strip), M1 tiering trigger marked fired, "frontier has one occupant" watch entry; body prose left as June time-capsule and still pre-dates Robert's no-em-dash voice rules (~130 em dashes pending a voice sweep). Sequel/companion to Scenario Planning (Current 4: Sovereignty) |
