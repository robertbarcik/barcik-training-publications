...
| The Mercantilism of Generative AI | `/mercantilism-of-genai/` | `_sources/mercantilism-of-genai/` (notes.md only) | ~9,000 words / 14 sections (June 2026); HTML-only hand-built, cloned from scenario-planning design system; 4 static CSS/SVG diagrams; new `.disclaimer-note` (red), `.bet`, `.coauthor-note` components; dated falsifiable bets + signed AI co-author's note + trigger-log time capsule. Sequel/companion to Scenario Planning (Current 4: Sovereignty) |
