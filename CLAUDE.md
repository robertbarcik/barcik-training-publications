...
| The Mercantilism of Generative AI | `/mercantilism-of-genai/` | `_sources/mercantilism-of-genai/` (notes.md only) | ~9,800 words / 14 sections (June 2026); HTML-only hand-built, cloned from scenario-planning design system; 6 static CSS/SVG diagrams (incl. "two futures" armed↔targeted spectrum + three-tier shield); new `.disclaimer-note` (red), `.bet`, `.coauthor-note` components; per-industry impact subsection, dated falsifiable bets + signed AI co-author's note + trigger-log time capsule. Sequel/companion to Scenario Planning (Current 4: Sovereignty) |
