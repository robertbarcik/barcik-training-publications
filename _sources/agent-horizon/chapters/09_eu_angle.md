...
**Human oversight requires integration hooks.** The agent can't be a black box. Humans must inspect, override, intervene. Frameworks with strong callback and hook models have an easier time satisfying this than frameworks expecting autonomous execution. What the human side of those hooks should look like in operations (approval gates, escalation paths, when a handoff helps and when it just slows everyone down) is the subject of our companion booklet [LLM-Human Interaction Design Patterns](/llm-human-interaction-patterns/).
...
> **July 2026 note.** Three months after this chapter was written, the argument stopped being hypothetical. The most capable US models were re-released behind gates: vetted companies only at the top tier (primarily American), a costly moderated API below it. Access to the frontier is now itself tiered by jurisdiction, which is precisely the exposure the routing pattern below is designed to absorb. How bloc-tiered access works, and where it likely goes next, is mapped in [The Mercantilism of Generative AI](/mercantilism-of-genai/#m-bloc).

> **September 2026 note.** The gates moved, in both directions. The nationality-scoped restriction on the most capable US models was lifted on June 30 with standing conditions (the vendor detects and reports malicious use, and coordinates standards for future models with the US government); the general-release tier has been available worldwide since July 1, while the unrestricted tier stays with vetted, mostly US organisations. On September 10 the EU's cybersecurity agency ENISA got access to that unrestricted tier after three months of negotiation, one version behind the current one. And OpenAI launched its own frontier model on September 3 behind a cyber-vetted cohort first, under its own preparedness rules rather than a government order. Read together: access to the frontier is now tiered by vetting and jurisdiction at both leading labs, and the tier a European buyer lands in is negotiated, agency by agency. The routing pattern below is the architecture that absorbs that.
