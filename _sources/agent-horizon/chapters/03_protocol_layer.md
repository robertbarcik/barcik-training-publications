...
MCP is a client-server protocol over a small JSON-RPC vocabulary. Until mid-2026, initial connection did one round of negotiation (`initialize`): protocol version, capabilities, identity. The 2026-07-28 revision replaced that with a stateless `server/discover` call and a per-request protocol-version field in `_meta`, so a plain HTTP load balancer can sit in front of a server without session affinity. After that, it's just method calls: `tools/list`, `tools/call`, `resources/list`, `resources/read`, `prompts/list`, `prompts/get`. The vocabulary is small on purpose. Most of the interesting design work happens inside the server (how you model your domain, how you write your tool descriptions, how you design your resource tree), not in the protocol itself.
...
> **September 2026 note.** The revision shipped as described, and the details matter for anyone writing servers this autumn. Sampling, roots, and logging are deprecated (kept for at least twelve months, so build nothing new on them; a server that needs a model now calls a model provider directly). The old HTTP+SSE transport is deprecated on the same runway in favour of Streamable HTTP. OAuth moved from Dynamic Client Registration to Client ID Metadata Documents. The Python SDK's 2.0 release renamed `FastMCP` to `MCPServer` and moved handlers into the constructor; a 2.x server still talks to pre-July clients. One layer up, the packaging question got an answer on August 6: Agent Plugins, an open format that bundles a skill and its MCP server configuration into one installable, run by a steering committee of Amazon, Cursor, Microsoft, OpenAI, and Vercel, with Google joining a week later, six clients supporting it at launch (ChatGPT, Codex, Cursor, GitHub Copilot, Kiro, VS Code) and Claude Code among those that followed. MCP carries the connection; plugins carry the know-how.
...
The would-be rivals have mostly folded in. IBM's Agent Communication Protocol merged into A2A in September 2025, and Google donated A2A to the Linux Foundation in 2025; on August 17, 2026 it formally became a hosted project of the same Agentic AI Foundation that governs MCP. At its one-year mark in April 2026 the protocol counted over 150 member organisations, production use at Microsoft, AWS, Salesforce, SAP, and ServiceNow, and a 1.0 release, the first stable specification, with cryptographically signed agent cards. A payments layer is already forming on top of it: the Agent Payments Protocol (AP2), backed by Google, Coinbase, Mastercard, and PayPal, standardises how agents transact money, with cryptographic mandates proving a human authorised the purchase.
