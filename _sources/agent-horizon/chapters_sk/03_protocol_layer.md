...
MCP je protokol klient–server nad malým slovníkom JSON-RPC. Do polovice roka 2026 urobilo prvotné pripojenie jedno kolo vyjednávania (`initialize`): verzia protokolu, schopnosti, identita. Revízia z 28. júla 2026 to nahradila bezstavovým volaním `server/discover` a poľom s verziou protokolu v `_meta` každej požiadavky, takže pred serverom môže stáť obyčajný HTTP load balancer bez lepenia sedení. Potom sú to už len volania metód: `tools/list`, `tools/call`, `resources/list`, `resources/read`, `prompts/list`, `prompts/get`. Slovník je malý zámerne. Väčšina zaujímavej návrhovej práce sa deje vnútri servera (ako modelujete svoju doménu, ako píšete popisy nástrojov, ako navrhujete strom zdrojov), nie v protokole samotnom.
...
> **Poznámka zo septembra 2026.** Revízia vyšla tak, ako je opísané, a detaily sú dôležité pre každého, kto túto jeseň píše servery. Sampling, roots a logging sú zastarané (ponechané aspoň dvanásť mesiacov, takže na nich nestavajte nič nové; server, ktorý potrebuje model, dnes volá poskytovateľa modelu priamo). Starý transport HTTP+SSE je zastaraný s rovnakou lehotou v prospech Streamable HTTP. OAuth prešlo z Dynamic Client Registration na Client ID Metadata Documents. Vydanie 2.0 Python SDK premenovalo `FastMCP` na `MCPServer` a presunulo obsluhu do konštruktora; server 2.x sa stále dohovorí s klientmi spred júla. O vrstvu vyššie dostala 6. augusta odpoveď otázka balenia: Agent Plugins, otvorený formát, ktorý zabalí skill a konfiguráciu jeho MCP servera do jednej inštalovateľnej jednotky, riadený výborom z Amazonu, Cursoru, Microsoftu, OpenAI a Vercelu, s Googlom, ktorý sa pridal o týždeň, šiestimi klientmi pri spustení (ChatGPT, Codex, Cursor, GitHub Copilot, Kiro, VS Code) a Claude Code medzi tými, čo nasledovali. MCP nesie spojenie; pluginy nesú know-how.
...
Potenciálni rivali sa väčšinou pridali. Agent Communication Protocol od IBM sa v septembri 2025 zlúčil do A2A a Google daroval A2A nadácii Linux Foundation v roku 2025; 17. augusta 2026 sa formálne stal hosťovaným projektom tej istej Agentic AI Foundation, ktorá spravuje MCP. Na svoje prvé výročie v apríli 2026 protokol počítal vyše 150 členských organizácií, produkčné použitie v Microsofte, AWS, Salesforce, SAP a ServiceNow a vydanie 1.0, prvú stabilnú špecifikáciu, s kryptograficky podpísanými kartami agentov. Nad ním sa už formuje platobná vrstva: Agent Payments Protocol (AP2), podporovaný Googlom, Coinbase, Mastercardom a PayPalom, štandardizuje, ako agenti prevádzajú peniaze, s kryptografickými mandátmi dokazujúcimi, že nákup schválil človek.
