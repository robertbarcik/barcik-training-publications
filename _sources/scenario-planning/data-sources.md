...
- Decoder prose streamlined (Robert, 2026-09-13): the text after the chart no longer refers to previous editions or scores itself (the "preview of a 2027 generation" paragraph, the "Scored, September 2026" paragraph and the July postscript + correction were merged into one plain paragraph on the summer's two facts: safeguards are the product design; access is now negotiated tier by tier). Scoring of the retired July reading lives only in Current 1's log and Bet 1's interim strip. Cycles box title without the "reading" suffix.
- Naming (Robert, 2026-09-13): the current generation is "Fable / Astra" everywhere the text speaks in the present; "Spud" survives only as the codename fact in the decoder (shipped as GPT-5.5), in the verbatim July log entry, in the changelog quote, and in the two July bet claims, which keep their wording with an editorial gloss "(today's Fable / Astra generation)". The "Bel" rumour for the unreleased Navier–Stokes model is not used (unverified). Current 1's July + September bet paragraphs merged into one.
- Final pass (2026-09-13): estimated model sizes are drawn dashed (new `estimated:true` flag on the 2026 dot, lighter opacity), footnote says so. Mercantilism's "pacing proposal" entry gained an explicit caveat that the safety reading is at least as plausible as the lens reading. Decoder arrows (2/3/4 yr) deliberately kept; redraw at the next edition.

## 2026-09-13 — Second pass: the 30-day clause (Current 4) + Robert's field note (Current 8)

Prompted by Robert's read-through of the September log update. EN edited by script, SK re-aligned with `resegment.py`/`sk_apply.py` (10 new segments, all translated by Fable 5.1) + `segments.py inject`.

- **C4 new scene "The 30-day clause: the safeguard became the border"** + log entry "The safeguard became the border" (Moved) + Procurement card patch. Sources (all fetched 2026-09-13):
  - Anthropic platform docs "API and data retention": Fable 5/5.1 and Mythos 5/5.1 are Covered Models, 30-day retention, "not available under ZDR unless expressly authorized by Anthropic", rule "applies wherever Covered Models are offered" (Claude API incl. Claude Platform on AWS, Bedrock, Google Cloud Agent Platform, Microsoft Foundry), API error "your organization or workspace must have data retention enabled" — https://platform.claude.com/docs/en/manage-claude/api-and-data-retention
  - Anthropic Privacy Center "Data retention practices for Covered Models", effective 2026-06-09 — https://privacy.claude.com/en/articles/15425996-data-retention-practices-for-covered-models
  - Anthropic platform docs "Data residency": `inference_geo` accepts only `us`/`global`, no EU option — https://platform.claude.com/docs/en/manage-claude/data-residency
  - AWS Bedrock "Abuse detection": Fable 5/5.1 all traffic retained ≤30 days, flagged traffic human-reviewed by AWS; EFS-eligible customers ZDR through 2026-12-31; OpenAI Astra/GPT-5.x only classifier-flagged traffic ≤30 days, "eligible customers may request full ZDR" — https://docs.aws.amazon.com/bedrock/latest/userguide/abuse-detection.html
  - AWS Bedrock model cards Mythos 5.1 / Astra: EU regions Global Cross-Region only (in-region / geo cross-region "not supported") — https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-mythos-5-1.html
  - Anthropic "Developing Enterprise Frontier Safeguards with our customers" 2026-09-01: customer-held storage (S3 / Azure Blob / GCS) for the 30-day copy, "roll out to customers in phases, with the goal of making it broadly available later this fall", regulated-industry motivation, no training motive — https://www.anthropic.com/news/enterprise-frontier-safeguards
  - OpenAI developer docs "Data controls": abuse-monitoring logs "retained for up to 30 days"; Astra listed on ZDR-eligible endpoints (chat completions / responses), EU region requires ZDR / MAM / Eyes Off / Safety Retention; 10% data-residency uplift for models released after 2026-03-05 — https://developers.openai.com/api/docs/guides/your-data
  - Microsoft Learn: abuse monitoring 30 days, Modified Abuse Monitoring by application; "Some advanced Models sold by Azure may have more stringent criteria for turning off abuse monitoring" (generic, not Astra-specific) — https://learn.microsoft.com/en-us/azure/foundry/responsible-ai/openai/limited-access
  - Astra "no EU Data Zone in Azure Foundry at launch": secondary only (Requesty 2026-09-06 citing TechInspire / The New Stack), tagged `reported` in the text. The Register 2026-09-02 and Help Net Security 2026-09-02 on EFS (press).
  - **Not found / honest gaps**: no EU DPA, EDPB or Commission statement on frontier-model retention; no named EU enterprise declining the models over it; openai.com enterprise-privacy pages returned 403 (Astra ZDR wording taken from developers.openai.com only). Robert's premise "cancels the frontier for most EU companies" was softened in the text to "hard stop for regulated deployers, procurement burden and tier lag for the rest" — GDPR permits processor retention under a DPA.
- **C8 new author-note "Field note: my own company, one generation ahead of the payroll data"** (Robert's own account, September 2026): 2025 Opus/reasoning-agent year, two cancelled junior hires; summer 2026 Fable/Astra role-taking (autobiography print run: SNK ISBN 978-80-975431-0-5 assigned 2026-08-14, barcode via `print/ean_*.typ`, print files in `BOOK_autobiography/print/`), mid/senior no longer justifiable; leading-indicator claim of 1–2 years lag for the broader labour market. Links `/a-point-in-time/` (SK: `/a-point-in-time-sk/`) and `#scenario-4`.
